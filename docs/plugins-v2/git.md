...
- [GitHub Task](#github-task)
  ...
  - [Create Access Token From GitHub App Installation](#create-access-token-from-github-app-installation)
...
### Create Access Token From GitHub App Installation

The `createAppToken` action can be used to generate an access token from a GitHub
App installation.

**`createAppToken` Returned Fields:**
...
flows:
  default:
    - task: github
      in:
        action: createAppToken
        apiUrl: "https://github.example.com/api/v3"
        auth:
          appInstallationSecret: { org: "my-org", name: "my-github-app-secret" }
      out: gitHubOut
      
    # Can be used with other tasks and scripts
    - task: git
      in:
        action: "clone"
        auth:
          basic:
            ...
            password: "${gitHubOut.token}"
        # ...other git parameters...
```
